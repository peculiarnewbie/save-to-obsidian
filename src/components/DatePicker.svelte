<script lang="ts">
	...
	const selectDate = (date: number, offset?: boolean) => {
	...
	function updateRows() {
		...
		let prevMonthLastDay = dayjs(currentDate)
			.subtract(1, "month")
			.endOf("month")
			.date();
...
{#if isEditing}
	...
	>
		<div
			...
			class="flex flex-col text-base shadow-lg text-[#bababa] w-fit bg-[#363636] outline-none p-2 rounded-md"
		>
			<div class="flex justify-between px-1">
				...
				<div class="flex justify-end">
					<button
						class="py-1 px-2 rounded-md hover:bg-[#4e4e4e] text-left"
						on:click={() => changeMonth(-1)}
					>
						<CustomImage src={dateBack} alt="dateBack" width="7px" />
					</button>
					<button
						class="py-1 px-2 rounded-md hover:bg-[#4e4e4e] text-xs font-semibold"
						on:click={() => changeMonth(0)}>TODAY</button
					>
					<button
						class="py-1 px-2 rounded-md hover:bg-[#4e4e4e] text-left"
						on:click={() => changeMonth(1)}
					...
					{#each col as i, cIndex}
						<td>
							...
							{#if (cIndex < startDay && rIndex == 0) || rIndex * 7 + cIndex >= endDay}
								<button
									class="bg-[#363636] rounded-md hover:bg-[#3f3f3f] w-8 h-8 text-neutral-600"
									on:click={() => {
										selectDate(i, true);
									}}>{i}</button
								>
							...
							{:else if rIndex * 7 + cIndex == dayjs(field.value).date() + startDay - 1 && dayjs(field.value).month() == currentMonth && dayjs(field.value).year() == currentYear}
								<button
									class="bg-violet-600 rounded-md hover:bg-[#4e4e4e] w-8 h-8 text-white"
									on:click={() => {
										selectDate(i);
									}}>{i}</button
								>
							...
							{:else if rIndex * 7 + cIndex == dayjs(today).date() + startDay - 1 && dayjs(today).month() == currentMonth && dayjs(today).year() == currentYear}
								<button
									class="bg-[#363636] rounded-md hover:bg-[#4e4e4e] w-8 h-8 text-violet-500"
									on:click={() => {
										selectDate(i);
									}}>{i}</button
								>
							{:else}
								<button
									class={`bg-[#363636] rounded-md hover:bg-[#4e4e4e] w-8 h-8`}
									on:click={() => {
										selectDate(i);
									}}>{i}</button
								>
