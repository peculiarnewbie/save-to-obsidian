<script lang="ts">
    ...
    export let reorderCleanup = null;
    ...
    */
	...
	const handleDragging = (e:MouseEvent) => {
		...
		if(isBeingDragged){
            ...
            if(e.clientY > yParentRef + elementHeight && index != length - startingIndex) {
                reorder(1)
            }
		} 
	}
...
<div bind:this={parentElement} class="relative">
    <div bind:this={thisElement} class={`${isBeingDragged ? "absolute w-full" : "relative"}`} style={`will-change:transform; transform: translate3d(${isBeingDragged? `${-xDrag}px, ${-yDrag}px` : "0px, 0px"}, 0px); ${isBeingDragged? "z-index: 999999;" : ""}`}>
        <slot />
    </div>
</div>
