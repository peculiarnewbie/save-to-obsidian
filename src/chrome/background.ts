...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if(request.action === "download"){  
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.sendMessage(tabs[0].id, { action: "bgElementSelected", path: request.path, value: request.value }, (response) => {
        sendResponse(response);
      });
    });
    return;
    ...
    (async () => {
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        chrome.tabs.sendMessage(tabs[0].id, { action: "bgValuesUpdate", paths: request.paths }, (response) => {
          // Handle the response from the content script
          actualResponse = response;
          sendResponse(actualResponse);
        });
